...
public class UI_ColorPicker : MonoBehaviour {
	...
	{
		style2XButtonBlue.normal.textColor = UserBlobManager.GetComponent<UserBlobManager>().FontColor;
		ColorPicked = UserBlobManager.GetComponent<UserBlobManager>().FontColor;
		Enabled = myValue;
	}
    ...
    void OnGUI()
	{
		if(Enabled == true)
		{
			...
			style2XButtonBlue.normal.textColor = ColorPicked;
...
//			if (GUI.RepeatButton(new Rect(0, 500, ImageWidth, ImageHeight), colorPicker)) 
	        if (GUI.RepeatButton(new Rect((Screen.width * PosAndSizePercent.x), (Screen.height * PosAndSizePercent.y), (Screen.width * PosAndSizePercent.z), (Screen.width * PosAndSizePercent.z)), colorPicker)) 
			{
...
//				print (ColorPicked.ToString());
			...
			{
				UserBlobManager.GetComponent<UserBlobManager>().FontColor = ColorPicked;
