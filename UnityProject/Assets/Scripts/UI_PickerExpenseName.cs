...
public class UI_PickerExpenseName : MonoBehaviour {
...
//		this.GetComponent<GUITexture>().enabled = myValue;
	...
	public void UpdateExpenseNameList()
	{
		// get the list from the userblob
		this.pickerParams[0].column = new string[UserBlobManager.GetComponent<UserBlobManager>().UserAppData.ExpenseNameList.Count];
		for ( int i = 0; i < UserBlobManager.GetComponent<UserBlobManager>().UserAppData.ExpenseNameList.Count; i++)
		{
			this.pickerParams[0].column[i] = UserBlobManager.GetComponent<UserBlobManager>().UserAppData.ExpenseNameList[i]; 
		}
	}
