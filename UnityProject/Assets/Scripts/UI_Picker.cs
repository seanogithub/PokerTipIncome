...
public class UI_Picker : MonoBehaviour {

	// screeen space size parameters
	public Vector4 PosAndSizePercent = new Vector4(0.5f, 0.5f, 0.5f, 0.2f);
	...
	void OnGUI()
	{
		...
		if (debugMode){
			for (int i=0;i<touchAreas.Length; i++){
				...
				GUI.DrawTexture(new Rect(touchAreas[i].x, touchAreas[i].y, touchAreas[i].width, touchAreas[i].height),uiPickerBackground);
