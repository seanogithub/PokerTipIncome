...
public class UI_PickerTournamentName : MonoBehaviour {
...
//		this.GetComponent<GUITexture>().enabled = myValue;
	...
	public void UpdateTournamentNameList()
	{
		// get the list from the userblob
		this.pickerParams[0].column = new string[UserBlobManager.GetComponent<UserBlobManager>().UserAppData.TournamentNameList.Count];
		for ( int i = 0; i < UserBlobManager.GetComponent<UserBlobManager>().UserAppData.TournamentNameList.Count; i++)
		{
			this.pickerParams[0].column[i] = UserBlobManager.GetComponent<UserBlobManager>().UserAppData.TournamentNameList[i]; 
		}
	}
