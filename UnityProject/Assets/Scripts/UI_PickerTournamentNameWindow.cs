...
public class UI_PickerTournamentNameWindow : MonoBehaviour {
	...
	void OnGUI()
	{
		if (Enabled == true)
		{
			...
			if (GUI.Button( new Rect((Screen.width * 0.17f), (Screen.height * 0.65f), (Screen.width * 0.15f), (Screen.height * 0.05f)) ,"Del", style2XButtonBlue))
			{
				...
				{				
					PopUpYesNoDialog("Are you sure you want to delete this tournament name?", this.gameObject.GetComponent<UI_PickerTournamentNameWindow>(), "DeleteTournamentName");
//					RemoveData();
				}
			}
			...
			if (GUI.Button( new Rect((Screen.width * 0.68f), (Screen.height * 0.65f), (Screen.width * 0.15f), (Screen.height * 0.05f)) ,"New", style2XButtonBlue))
			{
				if(PopUpActive == false)
				{				
					UIManager.GetComponent<UI_Manager>().SwitchStates("EnterTournamentName");
				}
			}
			...
			if (GUI.Button( new Rect((Screen.width * 0.2f), (Screen.height * 0.85f), (Screen.width * 0.2f), (Screen.height * 0.1f)) ,"OK", style2XButtonBlue))
			{
				if(PopUpActive == false)
				{				
					myAlphaNumericValueString = this.GetComponent<UI_PickerTournamentName>().pickerParams[0].column[this.GetComponent<UI_PickerTournamentName>().pickerParams[0].selectedIndexReadOnly].ToString();
					PassDataOut();
	...
	void RemoveData()
	{
//		print (this.GetComponent<UI_PickerTournamentName>().pickerParams[0].column.Length.ToString());
//		print (this.GetComponent<UI_PickerTournamentName>().pickerParams[0].selectedIndexReadOnly.ToString());
		if(this.GetComponent<UI_PickerTournamentName>().pickerParams[0].selectedIndexReadOnly != this.GetComponent<UI_PickerTournamentName>().pickerParams[0].column.Length - 1)
		{
			UserBlobManager.GetComponent<UserBlobManager>().UserAppData.TournamentNameList.RemoveAt(this.GetComponent<UI_PickerTournamentName>().pickerParams[0].selectedIndexReadOnly);
			UserBlobManager.GetComponent<UserBlobManager>().UserAppData.TournamentNameList.TrimExcess();
	...
	public void YesNoDialog(string DialogState)
	{
		...
		case "DeleteTournamentName":
			...
			{
				print ("DeleteTournamentName cancel");
			}
			break;			
		}
