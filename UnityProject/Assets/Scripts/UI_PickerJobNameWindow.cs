...
public class UI_PickerJobNameWindow : MonoBehaviour {
	...
	void OnGUI()
	{
		if (Enabled == true)
		{
			...
			if (GUI.Button( new Rect((Screen.width * 0.17f), (Screen.height * 0.65f), (Screen.width * 0.15f), (Screen.height * 0.05f)) ,"Del", style2XButtonBlue))
			{
				...
				{				
					PopUpYesNoDialog("Are you sure you want to delete this job name?", this.gameObject.GetComponent<UI_PickerJobNameWindow>(), "DeleteJobName");
//					RemoveData();				
				}
			}
			...
			if (GUI.Button( new Rect((Screen.width * 0.68f), (Screen.height * 0.65f), (Screen.width * 0.15f), (Screen.height * 0.05f)) ,"New", style2XButtonBlue))
			{
				if(PopUpActive == false)
				{				
					UIManager.GetComponent<UI_Manager>().SwitchStates("EnterJobName");
				}
			}
	...
	void RemoveData()
	{
//		print (this.GetComponent<UI_PickerJobName>().pickerParams[0].column.Length.ToString());
//		print (this.GetComponent<UI_PickerJobName>().pickerParams[0].selectedIndexReadOnly.ToString());
		if(this.GetComponent<UI_PickerJobName>().pickerParams[0].selectedIndexReadOnly != this.GetComponent<UI_PickerJobName>().pickerParams[0].column.Length - 1)
		{
			UserBlobManager.GetComponent<UserBlobManager>().UserAppData.JobNameList.RemoveAt(this.GetComponent<UI_PickerJobName>().pickerParams[0].selectedIndexReadOnly);
			UserBlobManager.GetComponent<UserBlobManager>().UserAppData.JobNameList.TrimExcess();
