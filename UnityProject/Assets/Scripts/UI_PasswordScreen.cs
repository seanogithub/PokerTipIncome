...
public class UI_PasswordScreen : MonoBehaviour {
...
//			GUI.Label(new Rect((Screen.width * 0.05f), (Screen.height * 0.17f), (float)style.fontSize * AspectRatioMultiplier, (float)style.fontSize * AspectRatioMultiplier), myNumericValueLabel, styleBigger);
			...
			GUI.Label(new Rect((Screen.width * 0.50f), (Screen.height * 0.15f), (float)style.fontSize * AspectRatioMultiplier, (float)style.fontSize * AspectRatioMultiplier), "Enter 4 Digit Pass Code", styleBigger);
			...
			if (myNumericValueString != "")
			{
				...
				var CheckNumberValue = 0.0f;
				if(float.TryParse(myNumericValueString, System.Globalization.NumberStyles.Integer ,new CultureInfo("en-US"), out CheckNumberValue))
				{
...
//					NumPadKeyBoard.text = PreviousmyNumericValueString;
			...
			if (GUI.Button( new Rect((Screen.width * 0.6f), (Screen.height * 0.35f), (Screen.width * 0.2f), (Screen.height * 0.1f)) ,"Clear", style2XButtonBlue))
			{
				myNumericValueString = "";
				NumPadKeyBoard.text = "";
			}
...
*/		
	...
	void CheckPassword()
	{
		if(UserBlobManager.GetComponent<UserBlobManager>().UserAppData.PassCode == myNumericValueString)
		{
			NumPadKeyBoard = null;
